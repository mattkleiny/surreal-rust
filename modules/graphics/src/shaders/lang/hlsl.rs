...
impl ShaderProgram {
  ...
  pub fn from_hlsl(graphics: &GraphicsEngine, code: &str) -> Result<Self, ShaderError> {
    Self::from_code::<HLSL>(graphics, code)
  }

  /// Loads a [`ShaderProgram`] from the given raw HLSL shader code file.
  pub fn from_hlsl_path<'a>(graphics: &GraphicsEngine, path: impl ToVirtualPath) -> Result<Self, ShaderError> {
    Self::from_path::<HLSL>(graphics, path)
  }

  /// Loads a [`ShaderProgram`] from the given raw HLSL stream.
  pub fn from_hlsl_stream(graphics: &GraphicsEngine, stream: &mut dyn InputStream) -> Result<Self, ShaderError> {
