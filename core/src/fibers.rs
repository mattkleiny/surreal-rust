...
pub fn block_on<F: Future>(mut future: F) -> F::Output {
  ...
  impl Signal {
    ...
    fn wait(&self) {
      ...
      match *state {
        ...
        SignalState::Empty => {
          // Nothing has happened yet, and we're the only thread waiting (as should be the case!). Set the
          // state accordingly and begin polling the condvar in a loop until it's no longer telling
          // us to wait. The loop prevents incorrect spurious wakeups.
  ...
  // SAFETY: We shadow `fut` so that it cannot be used again. The future is now pinned to the stack
  // and will not be moved until the end of this scope. This is, incidentally, exactly what the
  // `pin_mut!` macro from `pin_utils` does.
  let mut future = unsafe { std::pin::Pin::new_unchecked(&mut future) };

  // Signal used to wake up the thread for polling as the future moves to completion. We need to use
  // an `Arc` because, although the lifetime of `fut` is limited to this function, the underlying IO
  // abstraction might keep the signal alive for far longer. `Arc` is a thread-safe way to allow
  // this to happen. TODO: Investigate ways to reuse this `Arc<Signal>`... perhaps via a `static`?
